...
def convert_audio_and_split_transcript(
    input_dir, source_name, target_name, output_dir, output_file
):
...
def download_and_process_datasets(directory, datasets):
  ...
  """
    ...
    tf.logging.info("Preparing LibriSpeech dataset: {}".format(",".join(datasets)))
...
def main(_):
    ...
    if FLAGS.train_only:
        download_and_process_datasets(
            FLAGS.data_dir, ["train-clean-100", "train-clean-360", "train-other-500"]
        )
    elif FLAGS.dev_only:
